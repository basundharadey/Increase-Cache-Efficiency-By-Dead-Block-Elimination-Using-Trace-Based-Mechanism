...
static enum dead_block_predictor dead_block_predictor = None;
...
/* dead-block table for the l1 cache */
//static struct cache_t *dead_block_table;
...
// JLinge Edit:
static unsigned int				/* latency of block access */
dead_block_table_access_fn(enum mem_cmd cmd,	/* access cmd, Read or Write */
	      md_addr_t baddr,			/* block address to access */
	      int bsize,			/* size of block to access */
	      struct cache_blk_t *blk,		/* ptr to block in upper level */
	      tick_t now)			/* time of access */
...
//JLinge edit: Get predictor type
  opt_reg_int(odb, "-DBP",
	      "Dead block predictor",
	      &dead_block_predictor, /* default */ None,
	      /* print */TRUE, /* format */NULL);
}
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	// JLinge Edit: Create dead block table
	cache_dl1->dead_block_table = cache_create("Dead-Block Table", 1<<(20), 1, FALSE,
			       /* usize */4, 8, cache_char2policy(c),
			       dead_block_table_access_fn, /* hit lat */ 0);
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
      ...
      if (RUU[RUU_head].ea_comp)
	{
	  ...
	  if (!LSQ[LSQ_head].completed)
	    {
		//JLinge Edit:
		if(LSQ[LSQ_head].stalled != 0)
			break;

		++LSQ[LSQ_head].stalled;

		/*
		if(dead_block_predictor == RefTrace || dead_block_predictor == BurstTrace) 
		{
			md_addr_t blk_addr = ((LSQ[LSQ_head].addr) & ~(cache_dl1)->blk_mask);
		
			word_t repl_addr = get_replaced_block_addr(cache_dl1, 
						blk_addr);
			word_t repl_encode = get_replaced_block_trace(cache_dl1, 
						blk_addr);

			//If valid trace and we haven't already processed this stall
			//Add/update dead block table
			if(repl_addr != -1)
			{	
				//fprintf(stderr, "Adding %x, %x\n", repl_addr, LSQ[LSQ_head].addr);
			...
			//if (repl_addr == -1 && repl_encode != -1)
			//	fprintf(stderr, "Can't add %x %x %x\n",blk_addr, repl_encode, LSQ[LSQ_head].PC);
		}//*/
	  ...
	  if ((MD_OP_FLAGS(LSQ[LSQ_head].op) & (F_MEM|F_STORE))
	      ...
	      if (fu)
		{
		  ...
		  if (cache_dl1)
			...
			//JLinge Edit:
			lat =
			cache_access_and_encode(cache_dl1, Write, (LSQ[LSQ_head].addr&~3),
				     NULL, 4, sim_cycle, NULL, NULL,
					rs->PC, dead_block_predictor,
					cache_dl1->dead_block_table);
...
*/
	...
	{
		reference_trace_prediction(cache_dl1, 
			dead_block_table, 
			LSQ[LSQ_head].addr,
			LSQ[LSQ_head].PC);
	}
	...
	{
		burst_trace_prediction(cache_dl1, 
			dead_block_table, 
			LSQ[LSQ_head].addr,
			LSQ[LSQ_head].PC);
	}//*/
		  ...
		  if (fu)
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  if (!load_lat)
				...
				//Jlinge Edit:
				load_lat =
				    cache_access_and_encode(cache_dl1, Read,
						 (rs->addr & ~3), NULL, 4,
						 sim_cycle, NULL, NULL,
						rs->PC, dead_block_predictor,
						cache_dl1->dead_block_table);
...
//JLinge Edit:
md_addr_t blk_addr = ((rs->addr) & ~(cache_dl1)->blk_mask);
	if(dead_block_predictor == RefTrace)
	{
		reference_trace_prediction(cache_dl1, 
			dead_block_table, 
