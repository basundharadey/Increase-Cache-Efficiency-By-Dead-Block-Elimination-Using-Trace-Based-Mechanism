...
struct cache_blk_t
{
  ...
  int is_dead;			//Used as a boolean to indicate if this block is dead
  word_t encoding;
  word_t prev_encoding;
  word_t prev_addr;
  byte_t added;
  byte_t prefetched;
...
// JLinge Edit:
enum dead_block_predictor {
  None,		/* Do not used predictor */
  RefTrace,	/* Use reference trace predictor */
  BurstTrace	/* Use burst trace predictor */
};
...
#define DEAD_BLOCK_TABLE_MAX_SATURATION 4
...
burst_trace_prediction(struct cache_t * cache_dl1,
				...
				md_addr_t addr,
				md_addr_t PC);
#define NUM_DBT_ENTRIES_ADDR 8
struct DBT_entry_addr
{
	word_t count;
	md_addr_t pfch_addr;
};
...
struct DBT_entry
{
	...
	struct DBT_entry_addr entries[NUM_DBT_ENTRIES_ADDR];
/*
	md_addr_t pfch_addr;
	word_t count;
*/
	...
	struct DBT_entry * next;
	struct DBT_entry * prev;
};

struct DBT_entry_link
{
	struct DBT_entry * head;
	struct DBT_entry * tail;
};

#define NUM_DBT_ENTRIES 4
...
//static struct cache_t *dead_block_table = NULL;
